...
function HubPlayerUpdate(id)
{
    ...
    if (PlayersList)
    {
        ...
        if (player_panel == null)
        {
            ...
            $.CreatePanel(`DOTAHeroImage`, player_portrait, "", {scaling: "stretch-to-cover-preserve-aspect", heroname : String(playerInfo.player_selected_hero), tabindex : "auto", class: "hero_image_interface", heroimagestyle : "portrait"});
            ...
            let player_key_count = $.CreatePanel("Label", player_keys_panel, "")
...
function UpdatePlayerSystem(table, key, data ) 
{
	if (table == "players_system") 
	{
        ...
        if (player_panel)
        {
            let player_block_health_percent_label = player_panel.FindChildTraverse("player_block_health_percent_label")
            if (player_block_health_percent_label)
            {
                player_block_health_percent_label.text = data.health
            }
            let player_block_health_percent = player_panel.FindChildTraverse("player_block_health_percent")
            if (player_block_health_percent)
            {
                player_block_health_percent.style.width = (100 - ((data.max_health-data.health)*100) / data.max_health) + "%"
            }
...
function notification_player_step(data)
{
    ...
    time_notification.text = Players.GetPlayerName(data.id) + " " + "Начинает свой ход"
